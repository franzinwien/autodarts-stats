...
class AutodartsStats {
    ...
    async loadTurns(mpIds) { if (!mpIds.length) return []; let r = []; for (let i = 0; i < mpIds.length; i += 50) { const { data } = await supabase.from('turns').select('id,points,round,match_player_id,created_at,score_remaining').in('match_player_id', mpIds.slice(i,i+50)); if (data) r.push(...data); } return r; }
    ...
    const matchData = matches.map(mp => { const t = byMp[mp.id] || []; const calcAvg = t.length ? t.reduce((a,b)=>a+b,0)/t.length : 0; return {...mp, calcAvg, isWin: mp.match.winner === mp.player_index }; });
