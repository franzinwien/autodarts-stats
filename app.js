// =====================================================
// AUTODARTS STATS PRO - Main Application v2.1
// Fixed: No nested joins (Supabase RLS compatible)
// =====================================================
...
class AutodartsStats {
    ...
    getFilteredMatches() {
        ...
        if (this.filters.time !== 'all') {
            ...
            matches = matches.filter(mp => new Date(mp.match.finished_at) >= cutoff);
        }
        if (this.filters.type) matches = matches.filter(mp => mp.match.type === this.filters.type);
        if (this.filters.variant) matches = matches.filter(mp => mp.match.variant === this.filters.variant);
        return matches.sort((a, b) => new Date(b.match.finished_at) - new Date(a.match.finished_at));
    }
    ...
    async sendMagicLink() {
        ...
        button.disabled = true; button.innerHTML = '<span>Sende...</span>';
        try {
    ...
    async handleAuthSuccess(user) {
        this.user = user;
        try {
            const { data: allowedUser, error } = await supabase.from('allowed_users').select('*').eq('email', user.email).single();
            if (error || !allowedUser) { alert('Zugang nicht erlaubt.'); await this.logout(); return; }
    ...
    async loadBaseData() {
        const { data: matchPlayers } = await supabase.from('match_players').select('*').eq('user_id', this.autodartsUserId);
        this.matchPlayerIds = matchPlayers?.map(mp => mp.id) || [];
        const matchIds = matchPlayers?.map(mp => mp.match_id) || [];
        const { data: matches } = await supabase.from('matches').select('*').in('id', matchIds);
        const { data: allPlayers } = await supabase.from('match_players').select('*').in('match_id', matchIds);
        this.opponentMap = {};
        allPlayers?.forEach(p => { if (p.user_id !== this.autodartsUserId && !this.opponentMap[p.match_id]) this.opponentMap[p.match_id] = p; });
        this.allMatches = matchPlayers?.map(mp => ({ ...mp, match: matches?.find(m => m.id === mp.match_id) })).filter(mp => mp.match) || [];
    }
    ...
    async loadOverviewData() {
        ...
        try {
            ...
            const filteredMpIds = matches.map(m => m.id);
            const totalMatches = matches.length;
            let wins = 0, totalCheckoutRate = 0, checkoutCount = 0;
            matches.forEach(mp => {
                if (mp.match.winner === mp.player_index) wins++;
                if (mp.checkout_rate && mp.checkout_rate > 0) { totalCheckoutRate += mp.checkout_rate; checkoutCount++; }
            });
            const winRate = totalMatches > 0 ? ((wins / totalMatches) * 100).toFixed(1) : 0;
            const avgCheckout = checkoutCount > 0 ? ((totalCheckoutRate / checkoutCount) * 100).toFixed(1) : '-';
            
            const { data: turns } = await supabase.from('turns').select('points, round, match_player_id, created_at').in('match_player_id', filteredMpIds.length > 0 ? filteredMpIds : ['none']);
            
            let totalPoints = 0, totalTurns = 0, first9Points = 0, first9Turns = 0, count180 = 0;
            turns?.forEach(t => {
                if (t.points !== null) {
                    totalPoints += t.points; totalTurns++;
            ...
            this.renderAverageChart(matches, turns);
            this.renderResultsChart(wins, totalMatches - wins);
            this.renderFirst9ComparisonChart(turns);
            this.renderScoringDistributionChart(turns);
    ...
    renderAverageChart(matches, turns) {
        ...
        if (!ctx) return;
        const turnsByMp = {};
        turns?.forEach(t => { if (t.points !== null) { if (!turnsByMp[t.match_player_id]) turnsByMp[t.match_player_id] = []; turnsByMp[t.match_player_id].push(t.points); } });
        ...
        let dataPoints = matchAverages;
        if (matchAverages.length > 50) {
            const byWeek = {};
            matchAverages.forEach(m => {
                const weekStart = new Date(m.date); weekStart.setDate(weekStart.getDate() - weekStart.getDay());
                const key = weekStart.toISOString().split('T')[0];
                if (!byWeek[key]) byWeek[key] = [];
                byWeek[key].push(m.avg);
            });
            dataPoints = Object.entries(byWeek).map(([date, avgs]) => ({ date: new Date(date), avg: avgs.reduce((a, b) => a + b, 0) / avgs.length })).sort((a, b) => a.date - b.date);
        }
        ...
        this.avgChart = new Chart(ctx, {
            type: 'line',
            data: { labels, datasets: [{ label: '3-Dart Average', data, borderColor: CONFIG.COLORS.green, backgroundColor: 'rgba(16, 185, 129, 0.1)', fill: true, tension: 0.3, pointRadius: dataPoints.length > 20 ? 2 : 4, pointBackgroundColor: CONFIG.COLORS.green }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { color: 'rgba(255,255,255,0.1)' }, ticks: { color: '#94a3b8', maxRotation: 45, font: { size: 10 } } }, y: { grid: { color: 'rgba(255,255,255,0.1)' }, ticks: { color: '#94a3b8' }, suggestedMin: 25, suggestedMax: 55 } } }
        });
    }
    ...
    renderResultsChart(wins, losses) {
        ...
        this.resultsChart = new Chart(ctx, {
            ...
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: '#94a3b8', padding: 16 } } }, cutout: '65%' }
        });
    }
    ...
    renderFirst9ComparisonChart(turns) {
        ...
        turns?.forEach(t => { if (t.points !== null) { if (t.round <= 3) { first9Total += t.points; first9Count++; } else { restTotal += t.points; restCount++; } } });
        ...
        this.first9Chart = new Chart(ctx, {
            type: 'bar',
            data: { labels: ['First 9', 'Rest'], datasets: [{ data: [first9Avg, restAvg], backgroundColor: [CONFIG.COLORS.green, CONFIG.COLORS.blue], borderRadius: 8 }] },
    ...
    renderScoringDistributionChart(turns) {
        ...
        turns?.forEach(t => { if (t.points === null) return; if (t.points === 180) s180++; else if (t.points >= 140) s140to179++; else if (t.points >= 100) s100to139++; else if (t.points >= 60) s60to99++; else if (t.points >= 40) s40to59++; else under40++; });
        if (this.scoringChart) this.scoringChart.destroy();
        this.scoringChart = new Chart(ctx, {
            type: 'doughnut',
            data: { labels: ['<40', '40-59', '60-99', '100-139', '140-179', '180'], datasets: [{ data: [under40, s40to59, s60to99, s100to139, s140to179, s180], backgroundColor: ['#64748b', '#94a3b8', '#3b82f6', '#8b5cf6', '#f59e0b', '#10b981'], borderWidth: 0 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { color: '#94a3b8', padding: 8, font: { size: 11 } } } }, cutout: '50%' }
        });
    }
    ...
    renderHighScoresChart(turns) {
        ...
        const labels = months.map(m => { const [y, mon] = m.split('-'); return new Date(y, mon - 1).toLocaleDateString('de-DE', { month: 'short' }); });
        if (this.highScoresChart) this.highScoresChart.destroy();
        this.highScoresChart = new Chart(ctx, {
            type: 'bar',
            data: { labels, datasets: [ { label: '180s', data: months.map(m => monthly[m].s180), backgroundColor: CONFIG.COLORS.green, borderRadius: 4 }, { label: '140+', data: months.map(m => monthly[m].s140), backgroundColor: CONFIG.COLORS.yellow, borderRadius: 4 }, { label: '100+', data: months.map(m => monthly[m].s100), backgroundColor: CONFIG.COLORS.blue, borderRadius: 4 } ] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top', labels: { color: '#94a3b8', padding: 8, font: { size: 10 } } } }, scales: { x: { stacked: true, grid: { display: false }, ticks: { color: '#94a3b8', font: { size: 10 } } }, y: { stacked: true, grid: { color: 'rgba(255,255,255,0.1)' }, ticks: { color: '#94a3b8' } } } }
        });
    }
    ...
    renderRecentMatches(matches) {
        ...
        tbody.innerHTML = matches.map(mp => {
            ...
            if (opponent) oppName = opponent.is_bot ? '🤖 Bot Lvl ' + Math.round((opponent.cpu_ppr || 40) / 10) : (opponent.name || 'Gegner');
            const avg = mp.average ? mp.average.toFixed(1) : '-';
            return '<tr><td>' + date.toLocaleDateString('de-DE') + '</td><td>' + oppName + '</td><td class="' + (isWin ? 'result-win' : 'result-loss') + '">' + (isWin ? '✅ Sieg' : '❌ Niederlage') + '</td><td>' + avg + '</td><td><span class="badge badge-' + (match.type || 'online').toLowerCase() + '">' + (match.type || 'Online') + '</span></td></tr>';
        }).join('');
    }
    ...
    async loadScoringData() {
        ...
        try {
            ...
            const { data: turns } = await supabase.from('turns').select('id, points').in('match_player_id', mpIds.length > 0 ? mpIds : ['none']);
            const turnIds = turns?.map(t => t.id) || [];
            const { data: throws } = await supabase.from('throws').select('segment_name, segment_bed, segment_number, turn_id').in('turn_id', turnIds.length > 0 ? turnIds.slice(0, 5000) : ['none']);
            
            const t20Area = throws?.filter(t => [20, 1, 5].includes(t.segment_number)) || [];
            ...
            turns?.forEach(t => { if (t.points === 180) c180++; else if (t.points >= 140) c140++; else if (t.points >= 100) c100++; });
            ...
            this.renderScoreFreqChart(turns);
    ...
    renderT20Chart(data) {
        ...
        this.t20Chart = new Chart(ctx, {
            type: 'bar',
            data: { labels: Object.keys(counts), datasets: [{ data: Object.values(counts), backgroundColor: ['#10b981', '#34d399', '#f59e0b', '#fbbf24', '#3b82f6', '#60a5fa', '#ef4444'], borderRadius: 6 }] },
    ...
    async loadCheckoutData() {
        ...
        try {
            ...
            matches.forEach(mp => { if (mp.checkout_rate && mp.checkout_rate > 0) { totalCheckout += mp.checkout_rate; checkoutCount++; } });
            const avgCheckout = checkoutCount > 0 ? ((totalCheckout / checkoutCount) * 100).toFixed(1) : '-';
            
            const { data: checkoutTurns } = await supabase.from('turns').select('id, points, score_remaining').in('match_player_id', mpIds.length > 0 ? mpIds : ['none']).eq('score_remaining', 0);
            const checkoutTurnIds = checkoutTurns?.map(t => t.id) || [];
            const { data: checkoutThrows } = await supabase.from('throws').select('segment_name, segment_bed, turn_id').in('turn_id', checkoutTurnIds.length > 0 ? checkoutTurnIds : ['none']).eq('segment_bed', 'Double');
            
            const doubleCount = {};
            checkoutThrows?.forEach(t => { const d = t.segment_name; doubleCount[d] = (doubleCount[d] || 0) + 1; });
            const sorted = Object.entries(doubleCount).sort((a, b) => b[1] - a[1]);
            const favDouble = sorted[0]?.[0] || '-';
            const total = checkoutThrows?.length || 0;
    ...
    renderCheckoutTable(doubles) {
        ...
        if (!tbody) return;
        const total = doubles.reduce((s, d) => s + d[1], 0);
        tbody.innerHTML = doubles.slice(0, 15).map(function(d) { return '<tr><td>-</td><td><strong>' + d[0] + '</strong></td><td>' + d[1] + '</td><td>' + ((d[1]/total)*100).toFixed(1) + '%</td></tr>'; }).join('');
    }
        ...
        try {
            ...
            if (tbody) {
                tbody.innerHTML = filtered.map(mp => {
                    const m = mp.match, d = new Date(m.finished_at), w = m.winner === mp.player_index;
                    ...
                    return '<tr><td>' + d.toLocaleDateString('de-DE') + ' ' + d.toLocaleTimeString('de-DE', {hour:'2-digit',minute:'2-digit'}) + '</td><td>' + oppName + '</td><td class="' + (w ? 'result-win' : 'result-loss') + '">' + (w ? '✅ Sieg' : '❌ Niederlage') + '</td><td>' + (mp.legs_won || 0) + '</td><td>' + (mp.average?.toFixed(1) || '-') + '</td><td>' + (m.variant || '-') + '</td><td><span class="badge badge-' + (m.type || 'online').toLowerCase() + '">' + (m.type || 'Online') + '</span></td></tr>';
                }).join('');
            }
        } catch (error) { console.error('Matches error:', error); }
        finally { this.hideLoading(); }
    }
    ...
    async loadHeatmapData() {
        ...
        try {
            const matches = this.getFilteredMatches();
            const mpIds = matches.map(m => m.id);
            const { data: turns } = await supabase.from('turns').select('id').in('match_player_id', mpIds.length > 0 ? mpIds : ['none']);
            const turnIds = turns?.map(t => t.id) || [];
            
            const target = document.getElementById('heatmap-target')?.value || 'all';
            let query = supabase.from('throws').select('segment_name, segment_number, segment_bed, coord_x, coord_y').in('turn_id', turnIds.length > 0 ? turnIds.slice(0, 10000) : ['none']).not('coord_x', 'is', null);
            ...
            const { data: throws } = await query.limit(10000);
            this.renderDartboard(throws || []);
            this.renderSegmentStats(throws || []);
    ...
    renderDartboard(throws) {
        ...
        [{ radius: r, color: '#1a1a2e' }, { radius: r * 0.85, color: '#252540' }, { radius: r * 0.65, color: '#1a1a2e' }, { radius: r * 0.45, color: '#252540' }, { radius: r * 0.15, color: '#10b981' }, { radius: r * 0.06, color: '#ef4444' }].forEach(ring => { ctx.beginPath(); ctx.arc(cx, cy, ring.radius, 0, Math.PI * 2); ctx.fillStyle = ring.color; ctx.fill(); });
        const scale = 180;
        throws.forEach(t => { if (t.coord_x != null && t.coord_y != null) { ctx.beginPath(); ctx.arc(cx + t.coord_x * scale, cy - t.coord_y * scale, 3, 0, Math.PI * 2); ctx.fillStyle = 'rgba(16, 185, 129, 0.6)'; ctx.fill(); } });
    }
    ...
    renderSegmentStats(throws) {
        ...
        if (!container) return;
        const counts = {};
        throws.forEach(t => { if (t.segment_name && t.segment_name !== 'Outside') counts[t.segment_name] = (counts[t.segment_name] || 0) + 1; });
        const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 10);
        const max = sorted[0]?.[1] || 1;
        container.innerHTML = sorted.map(function(item) { return '<div class="segment-stat"><span class="segment-name">' + item[0] + '</span><div class="segment-bar"><div class="segment-bar-fill" style="width:' + ((item[1]/max)*100) + '%"></div></div><span class="segment-count">' + item[1] + '</span></div>'; }).join('');
    }
    ...
    async loadOpponentsData() {
        ...
        try {
            ...
            if (tbody) {
                tbody.innerHTML = sortedOpps.slice(0, 30).map(function(o) {
                    const winRate = ((o.wins / o.matches) * 100).toFixed(0);
                    const lastDate = o.lastMatch ? new Date(o.lastMatch).toLocaleDateString('de-DE') : '-';
                    return '<tr><td>' + o.name + '</td><td>' + o.matches + '</td><td class="result-win">' + o.wins + '</td><td class="result-loss">' + (o.matches - o.wins) + '</td><td>' + winRate + '%</td><td>-</td><td>' + lastDate + '</td></tr>';
                }).join('');
            }
