// =====================================================
// AUTODARTS STATS PRO - Main Application
// =====================================================
...
class AutodartsStats {
    ...
    getDateFilter() {
        if (this.filters.time === 'all') return null;
    ...
    async loadOverviewData() {
        ...
        try {
            ...
            // Apply filters
            let matches = matchPlayers.filter(mp => mp.match);
            const dateFilter = this.getDateFilter();
            if (dateFilter) matches = matches.filter(mp => mp.match.finished_at >= dateFilter);
            ...
            matches.sort((a, b) => new Date(b.match.finished_at) - new Date(a.match.finished_at));
            
            // Calculate stats
            const totalMatches = matches.length;
            let wins = 0;
            matches.forEach(mp => { if (mp.match.winner === mp.player_index) wins++; });
            const winRate = totalMatches > 0 ? ((wins / totalMatches) * 100).toFixed(1) : 0;
            
            // Get turns for average
            const mpIds = matches.map(m => m.id);
            const { data: turns } = await supabase
                .from('turns')
                .select('points, round, match_player_id')
                .in('match_player_id', mpIds);
            ...
            // Checkout rate
            let totalCheckoutRate = 0, checkoutCount = 0;
            matches.forEach(mp => {
            ...
            // Charts
            await this.renderMonthlyAverageChart(turns);
            ...
            await this.renderHighScoresChart();
    ...
    async renderMonthlyAverageChart(turns) {
        ...
        const { data: allTurns } = await supabase
            .from('turns')
            .select('points, created_at, match_player:match_players!inner(user_id)')
            .eq('match_player.user_id', this.autodartsUserId)
            .not('points', 'is', null);
        ...
        allTurns?.forEach(t => {
            ...
            monthlyData[month].total += t.points;
        ...
        const data = sortedMonths.map(m => (monthlyData[m].total / monthlyData[m].count).toFixed(1));
        
        if (this.monthlyAvgChart) this.monthlyAvgChart.destroy();
        this.monthlyAvgChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels,
                datasets: [{
    ...
    renderScoringDistributionChart(turns) {
        ...
        this.scoringChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{ data: [under40, s40to59, s60to99, s100to139, s140to179, s180], backgroundColor: ['#64748b', '#94a3b8', '#3b82f6', '#8b5cf6', '#f59e0b', '#10b981'], borderWidth: 0 }]
            },
            options: {
                responsive: true,
                maintainAspectRatio: false,
    ...
    async renderHighScoresChart() {
        ...
        const labels = months.map(m => new Date(m + '-01').toLocaleDateString('de-DE', { month: 'short' }));
    ...
    renderRecentMatches(matches) {
        ...
        tbody.innerHTML = matches.map(mp => {
            ...
            if (opponent) {
                oppName = opponent.is_bot ? `🤖 Bot Lvl ${Math.round((opponent.cpu_ppr || 40) / 10)}` : (opponent.name || 'Gegner');
            }
    ...
    async loadScoringData() {
        ...
        try {
            const { data: throws } = await supabase
                .from('throws')
                .select('segment_name, segment_bed, segment_number, turn:turns!inner(match_player:match_players!inner(user_id))')
            ...
            const { data: turns } = await supabase
                .from('turns')
                .select('points, match_player:match_players!inner(user_id)')
                .eq('match_player.user_id', this.autodartsUserId)
                .not('points', 'is', null);
    ...
    renderScoreFreqChart(turns) {
        ...
        this.scoreFreqChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: { legend: { display: false } },
                scales: { x: { grid: { display: false }, ticks: { color: '#94a3b8', font: { size: 9 }, maxRotation: 45 } }, y: { grid: { color: 'rgba(255,255,255,0.1)' }, ticks: { color: '#94a3b8' } } }
            }
        });
    }
    ...
    async loadCheckoutData() {
        this.showLoading();
        try {
            const { data: checkoutThrows } = await supabase
                .from('throws')
                .select('segment_name, segment_bed, turn:turns!inner(score_remaining, points, match_player:match_players!inner(user_id))')
                .eq('turn.match_player.user_id', this.autodartsUserId)
                .eq('turn.score_remaining', 0)
                .eq('segment_bed', 'Double');
            
            const doubleCount = {};
            checkoutThrows?.forEach(t => {
                const d = t.segment_name;
                doubleCount[d] = (doubleCount[d] || 0) + 1;
            });
            ...
            // Highest checkout
            const scores = checkoutThrows?.map(t => t.turn.points).filter(p => p > 0) || [];
            ...
            this.renderCheckoutScoreChart(checkoutThrows);
    ...
    renderCheckoutScoreChart(checkouts) {
        ...
        this.checkoutScoreChart = new Chart(ctx, {
            ...
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: false } },
                scales: { x: { grid: { display: false }, ticks: { color: '#94a3b8' } }, y: { grid: { color: 'rgba(255,255,255,0.1)' }, ticks: { color: '#94a3b8' } } }
            }
        });
    }
    ...
    renderCheckoutTable(doubles) {
        ...
        tbody.innerHTML = doubles.slice(0, 15).map(([d, c]) => `<tr><td>-</td><td><strong>${d}</strong></td><td>${c}</td><td>${((c/total)*100).toFixed(1)}%</td></tr>`).join('');
    }
    ...
    async loadMatchesPage() {
        this.showLoading();
        try {
            const { data: mps } = await supabase.from('match_players').select('*, match:matches (*)').eq('user_id', this.autodartsUserId);
            
            if (Object.keys(this.opponentMap).length === 0) {
                const ids = mps.map(m => m.match_id);
                const { data: all } = await supabase.from('match_players').select('*').in('match_id', ids);
                all?.forEach(p => { if (p.user_id !== this.autodartsUserId && !this.opponentMap[p.match_id]) this.opponentMap[p.match_id] = p; });
            }
            
            let filtered = mps.filter(mp => mp.match);
            const dateFilter = this.getDateFilter();
            if (dateFilter) filtered = filtered.filter(mp => mp.match.finished_at >= dateFilter);
            if (this.filters.type) filtered = filtered.filter(mp => mp.match.type === this.filters.type);
            if (this.filters.variant) filtered = filtered.filter(mp => mp.match.variant === this.filters.variant);
            filtered.sort((a, b) => new Date(b.match.finished_at) - new Date(a.match.finished_at));
    ...
    async loadHeatmapData() {
        ...
        try {
            ...
            let query = supabase
                .from('throws')
                .select('segment_name, segment_number, segment_bed, coord_x, coord_y, turn:turns!inner(match_player:match_players!inner(user_id))')
                .eq('turn.match_player.user_id', this.autodartsUserId)
                .not('coord_x', 'is', null);
    ...
    renderDartboard(throws) {
        ...
        // Rings
        [{ r: r, c: '#1a1a2e' }, { r: r*0.85, c: '#252540' }, { r: r*0.65, c: '#1a1a2e' }, { r: r*0.45, c: '#252540' }, { r: r*0.15, c: '#10b981' }, { r: r*0.06, c: '#ef4444' }]
            .forEach(ring => { ctx.beginPath(); ctx.arc(cx, cy, ring.r, 0, Math.PI*2); ctx.fillStyle = ring.c; ctx.fill(); });
    ...
    renderSegmentStats(throws) {
        ...
        container.innerHTML = sorted.map(([name, count]) => `
            <div class="segment-stat">
                ...
                <div class="segment-bar"><div class="segment-bar-fill" style="width:${(count/max)*100}%"></div></div>
    ...
    async loadOpponentsData() {
        ...
        try {
            ...
            mps.forEach(mp => {
                if (!mp.match) return;
                const opp = all?.find(p => p.match_id === mp.match_id && p.user_id !== this.autodartsUserId);
                if (!opp) return;
